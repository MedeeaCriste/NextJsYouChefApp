...
export default function ResultElement({title, prepTime, ingredients, prepDetails, category}:any) {
  ...
  return (
    <div className={`flex gap-4 p-4 bg-white shadow-md ${openDivClasses.divClass}`}>
        <Image className='w-[180px] h-[200px] rounded-sm aspect-square object-cover shadow-xl ml-[-4%] border-2 border-main' src={`/category_images/category_${category}.jpg`} alt='img recipe' width={300} height={300}/>
        ...
        <button onClick={handleClick} className={`${openDivClasses.buttonClasses} border-2 px-8 py-2 rounded self-end font-semibold mb-[-3%] `}>{openDivClasses.buttonContent}</button>
